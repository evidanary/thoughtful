...
const SocialMedia = () => {
  ...
  const queuedPosts = posts.filter(post => 
    post.status.toLowerCase() !== 'posted' && !scheduledPosts[post.id]
  );
  ...
  // Optimal posting times by day of week (0=Sunday, 1=Monday, etc.) in PST
  // Based on social media engagement research
  const getOptimalPostingTime = (dayOfWeek) => {
    const optimalTimes = {
      0: { hour: 10, minute: 0 },  // Sunday: 10:00 AM - morning relaxation scroll
      1: { hour: 11, minute: 0 },  // Monday: 11:00 AM - settling into work week
      2: { hour: 9, minute: 0 },   // Tuesday: 9:00 AM - highest engagement day
      3: { hour: 11, minute: 0 },  // Wednesday: 11:00 AM - mid-week peak
      4: { hour: 12, minute: 0 },  // Thursday: 12:00 PM - lunch hour engagement
      5: { hour: 10, minute: 0 },  // Friday: 10:00 AM - before weekend mode
      6: { hour: 9, minute: 0 },   // Saturday: 9:00 AM - morning scroll time
    };
    return optimalTimes[dayOfWeek] || { hour: 10, minute: 0 };
  };
  ...
  const copyScheduleToClipboard = () => {
    const scheduledEntries = Object.entries(scheduledPosts)
      .map(([postId, date]) => {
        ...
        // Parse the scheduled date and get day of week
        const [year, month, day] = date.split('-').map(Number);
        const scheduledDate = new Date(year, month - 1, day);
        const dayOfWeek = scheduledDate.getDay();
        
        // Get optimal posting time for this day
        const optimalTime = getOptimalPostingTime(dayOfWeek);
        
        // Create ISO 8601 timestamp in PST (UTC-8)
        // Format: YYYY-MM-DDTHH:MM:SS-08:00
        const hourStr = String(optimalTime.hour).padStart(2, '0');
        const minuteStr = String(optimalTime.minute).padStart(2, '0');
        const timestamp = `${date}T${hourStr}:${minuteStr}:00-08:00`;
        
        // Format: Timestamp\tContent\tImages\tStatus (always "Pending")
        return `${timestamp}\t${formattedContent}\t${formattedImages}\tPending`;
      })
      .filter(Boolean);
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ 
            ...
            Copy data from Google Sheets with columns: <strong>Date Added</strong>, <strong>Post Content</strong>, <strong>Image List</strong>, <strong>Status</strong>
          </p>
          <textarea
            value={pastedContent}
          ...
          {posts.length > 0 && (
            <div style={{
              ...
              <span>✓</span>
              Parsed {posts.length} post{posts.length !== 1 ? 's' : ''} ({queuedPosts.length} in queue)
            </div>
          )}
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      top: '12px',
                      right: '12px',
                      backgroundColor: post.status.toLowerCase() === 'draft' ? '#6e7681' :
                                      post.status.toLowerCase() === 'scheduled' ? '#1f6feb' :
                                      '#f97316',
                      color: 'white',
                      padding: '2px 8px',
                      borderRadius: '4px',
          ...
          <>
            <div style={{
              ...
              <button
                ...
                style={{
                  padding: '12px 24px',
                  backgroundColor: activeTab === 'calendar' ? '#f97316' : '#21262d',
                        ...
                        <div style={{ 
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ 
                              ...
                              <span style={{
                                backgroundColor: post.status.toLowerCase() === 'posted' ? '#238636' :
                                                post.status.toLowerCase() === 'draft' ? '#6e7681' :
                                                post.status.toLowerCase() === 'scheduled' ? '#1f6feb' :
                                                '#f97316',
                                color: 'white',
                                padding: '4px 10px',
                              ...
                              {getScheduledDate(post.id) && (
                                <span style={{
                                  ...
                                  📅 Scheduled: {getScheduledDate(post.id)} 12:00 PM
                                </span>
                              )}
                            </div>
                ...
                <div style={{ padding: '24px' }} ref={calendarRef}>
                  ...
                  <div style={{ 
                    ...
                    <button
                      ...
                      disabled={Object.keys(scheduledPosts).length === 0 || copied}
                      style={{
                        padding: '12px 24px',
                        backgroundColor: copied ? '#166534' : 
                                        Object.keys(scheduledPosts).length > 0 ? '#238636' : '#21262d',
                        color: copied ? 'white' :
                               Object.keys(scheduledPosts).length > 0 ? 'white' : '#8b949e',
                        border: 'none',
                        borderRadius: '8px',
                        fontSize: '14px',
                        fontWeight: '600',
                        cursor: Object.keys(scheduledPosts).length > 0 && !copied ? 'pointer' : 'not-allowed',
                        display: 'flex',
                        alignItems: 'center',
                        gap: '8px',
                          ...
                          {month.days.map((dayData, dayIndex) => {
                            ...
                            return (
                              <div
                                ...
                                onDragEnter={(e) => handleDragEnter(e, dayData.date)}
                                onDragLeave={handleDragLeave}
                                onDrop={(e) => handleDrop(e, dayData.date)}
                                style={{
                                  minHeight: '80px',
                                  backgroundColor: isHovered ? '#2d1810' :
                                                  dayData.isToday ? '#1c3a5e' : 
                                                  hasScheduledPosts ? '#1a2f1a' : '#21262d',
                                  borderRadius: '8px',
                                  border: isHovered ? '2px dashed #f97316' :
                                         dayData.isToday ? '2px solid #1f6feb' : '1px solid #30363d',
                                  padding: '8px',
                              ...
                              >
                                <div style={{
                                  ...
                                  color: dayData.isToday ? '#58a6ff' : '#8b949e',
                                  marginBottom: '6px'
                                }}>
                                  {dayData.day}
                                </div>
        ...
        {posts.length === 0 && (
          <div style={{
            ...
            <p style={{ 
              ...
              Paste content from Google Sheets above to get started. Make sure your data has columns for Date Added, Post Content, Image List, and Status.
            </p>
          </div>
        )}
      </div>
